...
const PatientAdmissionTable: FunctionComponent<IOwnProps> = ({
  ...
  const label = {
    ...
    diagnosisOut: t("admission.diagnosisOut"),
    complicationDiagnosis: t("admission.complicationDiagnosis"),
    othersInformation: t("admission.othersInformation"),
  ...
  const formatDataToDisplay = (data: AdmissionDTO[]) => {
    return data.map((item) => {
      return {
        ...
        diagnosisIn: item.diagnosisIn?.map((d) => d.description).join(", ") ?? "",
        transUnit: item.transUnit,
        fhu: item.fhu,
        ward: item.ward?.description ?? "",
        anamnesis: item.anamnesis ?? "",
        disType: item.disType?.description ?? "",
        diagnosisOut: item.diagnosisOut?.map((d) => d.description).join(", ") ?? "",
        complicationDiagnosis: item.complicationDiagnosis?.map((d) => d.description).join(", ") ?? "",
        othersInformation: item.othersInformation ?? "",
