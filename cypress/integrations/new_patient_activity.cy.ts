...
describe("NewPatientActivity spec", () => {
  ...
  it('should pass the "submit" button label', () => {
    cy.dataCy("patient-data-submit-button").should("contain", "Submit");
  });
  ...
  const selectBirthDate = (year = 2017, month = 1, day = 1) => {
    cy.byId("birthDate").click();

    cy.get('[aria-labelledby="dialog-year-label"]').click();
    cy.contains('[role="option"]', year.toString()).click();

    cy.get('[aria-labelledby="dialog-month-label"]').click();
    cy.contains('[role="option"]', /^1\s*-/).click();

    cy.get('[aria-labelledby="dialog-day-label"]').click();
    cy.contains('[role="option"]', day.toString()).click();
